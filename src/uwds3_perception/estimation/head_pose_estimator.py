...
class HeadPoseEstimator(object):
    def __init__(self, face_3d_model_filename):
        ...
        self.__rotate_model(-1.57, 3.14, 0)

    def __check_consitency(self, trans, rot):
        consistent = True
        if trans[2][0] > MAX_DIST or trans[2][0] < 0 or trans[0][0] > MAX_DIST or trans[1][0] > MAX_DIST:
            consistent = False
        return consistent

    def __rotate_model(self, rx, ry, rz):
        r_mat = euler_matrix(rx, ry, rz, "rxyz")[:3, :3]
        self.model_3d = np.dot(self.model_3d, r_mat)

    def estimate(self, faces, camera_matrix, dist_coeffs):
        """Estimate the head pose of the given face (y forward)"""
        for f in faces:
            if f.is_confirmed():
                ...
                if f.pose is not None:
                    ...
                    success = self.__check_consitency(trans, rot)
                else:
