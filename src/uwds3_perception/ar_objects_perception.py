...
class ARObjectsPerception(object):
    def __init__(self):
        ...
        for entity_id, entity in self.config.items():
            ...
            track.shape.type = uwds3_msgs.msg.PrimitiveShape.MESH
            ...
            track.shape.pose.position.x = position["x"]
            track.shape.pose.position.y = position["y"]
            track.shape.pose.position.z = position["z"]
            track.shape.pose.orientation.x = q[0]
            track.shape.pose.orientation.y = q[1]
            track.shape.pose.orientation.z = q[2]
            track.shape.pose.orientation.w = q[3]
            track.shape.mesh_resource = "file://"+self.resource_directory+self.config[entity_id]["mesh_resource"]
